function [LL, LL_pxz, LL_pz]=LogLikePLRNN3(A,W,C,S,Inp_,mu0_,B,G,h,X_,Z_,lam)
if nargin<12, lam=0; end    % regularization on W
%
...
for i=1:ntr   % cycle through all trials
    ...
    LL=LL-1/2*(LL0+LL1+LL2+T(i)*sum(log(diag(G)))+T(i)*sum(log(diag(S))));  % works only for diag S,G
end;
LL_pxz=LL2;
LL_pz=LL0+LL1;
