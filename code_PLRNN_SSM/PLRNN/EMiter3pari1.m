function [mu0sv,Bsv,Gsv,Asv,Wsv,Csv,hsv,Ssv,EziSv,VestSv,EphiziSv,EphizijSv,EziphizjSv,LL,mu0allSv,LL_pxz,LL_pz]= ...
    EMiter3pari1(CtrPar,A_est,W_est,C_est,S_est,Inp,mu0all,B_est,G_est,h_est,X,XZspl,Lb,Ub,lam)
...
while (i==1 || LLR>tol*abs(LL(1)) || LLR<0) && i<MaxIter
    ...
    [mu0_est,B_est,G_est,W_est,A_est,S_est,C_est,h_est]=ParEstPLRNNi1(EV,XZspl,S0,C0,B0,G0,Lb,Ub,lam);
    
    % compute log-likelihood (alternatively, use ELL output from ParEstPLRNN)
    EziAll=cell2mat(Ezi);
    if iscell(X{1})
        LL(i)=LogLikePLRNN3(A_est,W_est,C_est,S_est,Inp{1:end},mu0_est,B_est,G_est,h_est,X{1:end},EziAll,lam);
        for np=1:NP, mu0all{np}=mu0_est(Ntr(np)+1:Ntr(np+1)); end
    else
        [LL(i),LL_pxz(i), LL_pz(i)]=LogLikePLRNN3(A_est,W_est,C_est,S_est,Inp,mu0_est,B_est,G_est,h_est,X,EziAll,lam);
        mu0all=mu0_est;
    end
