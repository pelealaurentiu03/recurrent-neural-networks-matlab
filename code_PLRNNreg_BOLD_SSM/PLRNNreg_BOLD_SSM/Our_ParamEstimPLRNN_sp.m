...
% Modified by Lia on 24.12.2025 – E/I sign projection on recurrent weights (Step 2.3)
% =========================================================================
...
for i=1:ntr
    ...
    hEzi0 = H*Ez0;
    hEzi_mat = reshape(hEzi0, m, []); 
    
    hF1 = hF1 + X{i}*hEzi_mat';
    hF9 = hF9 + rp'*hEzi_mat';
    hE1p = hE1p + (hEzi_mat * hEzi_mat'); 
...
% =====================================================================
% 4e. E/I SIGN CONSTRAINT (Step 2.3) - Projection
% Excitatory rows: W(i,*) >= 0
% Inhibitory rows: W(i,*) <= 0
% =====================================================================
if isfield(net,'reg') && isfield(net.reg,'use_EI') && net.reg.use_EI ...
        && isfield(net.reg,'EI_sign') && numel(net.reg.EI_sign) == m && m > 1

    idx = [2:i-1, i+1:m];  % exclude diagonal position i

    if net.reg.EI_sign(i) > 0
        % Excitatory: clamp negatives to 0
        theta(idx) = max(theta(idx), 0);
    else
        % Inhibitory: clamp positives to 0
        theta(idx) = min(theta(idx), 0);
    end
end
