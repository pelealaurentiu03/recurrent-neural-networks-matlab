...
for i=1:m
    ...
    % =========================================================================
    % <<< ADDED: REQUIREMENT 2.2 MODULARITY PRIOR >>>
    % =========================================================================
    if isfield(net.reg, 'use_modularity') && net.reg.use_modularity && isfield(net.reg, 'modularity_mask')
        ...
        sigma_val = net.Sigma(i,i); 
        
        valid_indices = find(Mask(:,i));
        
        for j = 1:m
             mult_factor = row_mask(j); 
             
             if mult_factor > 1 && j ~= i
                 
                 global_idx_W_ij = m + j;
                 
                 local_idx = find(valid_indices == global_idx_W_ij);
                 
                 if ~isempty(local_idx)
                     extra_penalty = sigma_val * base_tau * (mult_factor - 1);
                     el(local_idx, local_idx) = el(local_idx, local_idx) + extra_penalty;
                 end
             end
        end
    end
